...
Опційні змінні середовища:
  ...
  INSTAGRAM_USER_ID          — ID Instagram Business акаунту
  ...
  GOOGLE_DRIVE_FOLDER_ID     — ID папки в Google Drive для архіву контенту
"""
...
def save_to_drive(image_bytes: bytes, instagram_text: str, date_str: str) -> dict:
    ...
    image_link = upload_to_drive(
        service, image_bytes,
        f'LUNA_Instagram_{date_str}.png', 'image/png', folder_id,
    )
    text_link = upload_to_drive(
        service, instagram_text.encode('utf-8'),
        f'LUNA_Instagram_{date_str}.txt', 'text/plain', folder_id,
    )
...
def main():
    ...
    drive_enabled     = bool(os.environ.get('GOOGLE_SERVICE_ACCOUNT_JSON') and os.environ.get('GOOGLE_DRIVE_FOLDER_ID'))
    date_str = datetime.now().strftime('%Y-%m-%d')

    print(f'📋 Активні канали:')
    print(f'  Telegram канал 1 : ✅')
    print(f'  Telegram канал 2 : {"✅" if channel_id_2 else "⏭️  не налаштовано"}')
    print(f'  Instagram        : {"✅" if instagram_enabled else "⏭️  не налаштовано"}')
    print(f'  Google Drive     : {"✅" if drive_enabled else "⏭️  не налаштовано"}')
    print()
    ...
    # 3. Адаптація для Instagram (якщо Instagram або Drive активні)
    instagram_text = None
    if instagram_enabled or drive_enabled:
    ...
    # 7. Публікація в Instagram (якщо налаштовано)
    ...
    # 8. Збереження в Google Drive (якщо налаштовано)
    if drive_enabled:
        print('💾 Збереження Instagram контенту в Google Drive...')
        try:
            drive_links = save_to_drive(image_bytes, instagram_text, date_str)
            print(f'🖼️  Зображення: {drive_links["image_link"]}')
            print(f'📝 Текст:       {drive_links["text_link"]}')
        except Exception as e:
            print(f'❌ Google Drive — помилка: {e}')
